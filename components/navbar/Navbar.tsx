...
export default function Navbar(props: { navItems: NavItem[]; tinaProps: NavbarProps }) {
    ...
    return (
        <header style={{ boxShadow: 'rgba(0, 0, 0, 0.16) 0px 1px 4px' }} className="bg-white border-b-2 border-special-green sticky top-0 z-20">
                    ...
                    {items.map((item, i) => {
                        switch (item.__typename) {
                            case 'NavbarTop_level_itemsSubmenu':
                                ...
                                return (
                                    <Popover key={encodeURIComponent(item.text)} className="relative">
                                        {({ open, close }) => (
                                            <>
                                                                          ...
                                                                          return (
                                                                              ...
                                                                              >
                                                                                  <span className="flex items-center gap-1">
                                                                                      {locale === 'en' ? child.external_link_text_en : child.external_link_text}
                                                                      ...
                                                                      case 'NavbarTop_level_itemsSubmenuItemsPagelink':
                                                                          ...
                                                                          return (
                                                                              <Link
                                                                                  ...
                                                                                  className={classNames(
                                                                                      asPath === '/' + child.page._sys.breadcrumbs.join('/')
                                                                                          ? 'bg-gray-100'
                                                                                          : '',
                                                                                      'block rounded-lg px-3 py-2 text-sm font-semibold leading-6 text-gray-900 hover:bg-gray-100',
                                                                                  )}
                                                                                  onClick={() => close()}
                                                                              >
                                                                                  {locale === 'en' ? child.page.titleEnglish : child.page.title}
                                                                              </Link>
                                                                          );
                                                                  }
                                                              })
                            ...
                            case 'NavbarTop_level_itemsLink':
                                if (item.link == null || item.external_link_text == null || item.external_link_text_en == null) {
                                    return null;
                                }
                                return (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className={classNames('leading-6 flex items-center text-sm font-semibold gap-1')}
                                    >
                                        {locale === 'en' ? item.external_link_text_en : item.external_link_text} <ArrowTopRightOnSquareIcon className="h-4" />
                                    </a>
                                );
